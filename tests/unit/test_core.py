...
class TestToCpeVendor:
    def test_dicom_philips_uid(self):
        env = {"pipeline_3": {"dicom_association": [
...
class TestSignalObservation:
    def test_minimal_construction(self):
        sig = SignalObservation(
            pipeline=1, protocol="WS_DISCOVERY",
            src_mac="00:09:fb:bd:75:6d", timestamp=1000.0
        )
...
class TestFusionOutput:
    def test_construction(self):
        fuse = FusionOutput(
            host_id="aa:bb:cc:dd:ee:ff", mac="AA:BB:CC:DD:EE:FF",
            ip="10.0.0.1", path="FUSED",
            device_class="CT", confidence="HIGH",
    ...
    def test_contradiction_flag(self):
        fuse = FusionOutput(
            host_id="aa:bb:cc:dd:ee:ff", mac="AA:BB:CC:DD:EE:FF",
            ip="10.0.0.1", path="FUSED",
            device_class=None, confidence="MEDIUM",
