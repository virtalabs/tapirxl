...
def _eth_ip_udp(
    *,
    ...
    return (
        ...
        / IP(src=ip_src, dst=ip_dst, ttl=1 if ip_dst.startswith("239.") else 64)
...
def build_dhcp_offer(*, client_mac: str, offered_ip: str, xid: int, hostname: str) -> object:
...
def build_dhcp_ack(*, client_mac: str, offered_ip: str, xid: int, hostname: str) -> object:
    return _dhcp_server_reply(
        msg_type="ack",
        xid=xid,
        client_mac=client_mac,
...
def build_arp_reply(*, src_mac: str, src_ip: str, target_mac: str, target_ip: str) -> object:
    from scapy.layers.l2 import ARP, Ether

    return Ether(src=src_mac, dst=target_mac) / ARP(
        op=2, hwsrc=src_mac, psrc=src_ip, hwdst=target_mac, pdst=target_ip
    )
...
def build_tls_client_hello(client: dict, server: dict, sport: int, sni: str) -> list[object]:
    init = TcpState(
    ...
    seg, _ = tcp_psh_exchange(sender="client", tcpst=tcpst, pdu=_tls_client_hello_plaintext(sni))
    return [*hs, *seg]
...
def build_smb2_session_setup_ntlmssp(
    *,
    ...
    auth = build_ntlmssp_authenticate(domain=domain, username=username, workstation=client_hostname)
...
def _build_a_associate_rq_pdu() -> bytes:
    try:
        ...
        raise SystemExit("Install pynetdicom+pydicom: pip install pydicom pynetdicom") from exc
...
def scenario_brilliance_to_ccp() -> list[object]:
    ...
    seg, tcpst = tcp_psh_exchange(sender="client", tcpst=tcpst, pdu=_build_a_associate_rq_pdu())
    pkts_out.extend(seg)

    seg, tcpst = tcp_psh_exchange(sender="server", tcpst=tcpst, pdu=_build_a_associate_ac_pdu())
    pkts_out.extend(seg)
...
def scenario_win7_ct_console() -> list[object]:
    ...
    out: list[object] = [
        ...
        *build_tls_client_hello(w, TLS_SENTINELONE, sport=49_302, sni="usea1-016.sentinelone.net"),
        *build_tls_client_hello(w, TLS_MICROSOFT, sport=49_303, sni="events.data.microsoft.com"),
...
def scenario_ccplatform_dhcp_llmnr() -> list[object]:
    ...
    out.append(build_llmnr_response(CCPLATFORM["mac"], CCPLATFORM["ip"], CCPLATFORM["name"]))
    return out
...
def scenario_acmews02() -> list[object]:
    ...
    out.append(build_llmnr_response(ACMEWS02["mac"], ACMEWS02["ip"], ACMEWS02["name"]))
    return out
...
def scenario_pacsarch01() -> list[object]:
    ...
    out.append(build_llmnr_response(PACSARCH01["mac"], PACSARCH01["ip"], PACSARCH01["name"]))
    return out
